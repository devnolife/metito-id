          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {stats.map((stat, index) => (
              <Reveal key={index} delay={index % 4} className="text-center group">
                <div className="inline-flex items-center justify-center w-20 h-20 bg-gold/15 text-[var(--gold)] border border-gold/30 rounded-2xl mb-6 group-hover:scale-110 transition-transform">
                  {stat.icon}
                </div>
                <div className="font-display text-4xl md:text-5xl font-bold mb-2 text-[var(--navy)]">{stat.value}</div>
                <div className="text-[var(--body-muted)] font-medium">{stat.label}</div>
              </Reveal>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {loading && (
            <div className="flex justify-center items-center py-20">
              ...
              <span className="ml-2 text-[var(--body-muted)]">Memuat testimoni...</span>
            </div>
          )}
                  ...
                  <Card className="group h-full rounded-[1.25rem] bg-[var(--surface)] border border-[var(--hairline)] shadow-[0_24px_60px_-28px_rgba(11,28,48,0.18)] transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_34px_70px_-24px_rgba(11,28,48,0.28)]">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <Quote className="w-8 h-8 text-lime/40 group-hover:text-[var(--lime)] transition-colors" />
                      </div>

                      <p className="text-[var(--body-text)] mb-6 leading-relaxed text-sm">
                        "{testimonial.content}"
                      </p>
                      ...
                      <div className="border-t border-[var(--hairline)] pt-4">
                        <div className="flex items-center gap-4 mb-4">
                          <Avatar className="w-12 h-12">
                            ...
                            <AvatarFallback className="bg-gold/15 text-[var(--gold)] border border-gold/30 font-semibold">
                          ...
                          <div>
                            ...
                            {testimonial.position && (
                              <p className="text-sm text-[var(--body-muted)]">{testimonial.position}</p>
                            )}
                            {testimonial.company && (
              ...
              {testimonials.length === 0 && (
                <div className="col-span-full text-center py-16">
                  <div className="w-24 h-24 bg-[var(--surface-2)] rounded-full flex items-center justify-center mx-auto mb-6">
                    <Quote className="w-12 h-12 text-[var(--body-muted)]" />
                  </div>
                  <h3 className="font-display text-2xl font-bold text-white mb-2">Belum Ada Testimoni</h3>
                  <p className="text-[var(--body-muted)] mb-6">Testimoni pelanggan akan tampil di sini</p>
                </div>
              )}
            </div>
          )}
          ...
          <Reveal className="flex flex-wrap justify-center gap-4">
            {industries.map((industry, index) => (
              <Badge
                key={index}
                variant="outline"
                className="px-6 py-3 text-sm font-medium border-[var(--hairline)] text-[var(--navy)] hover:bg-[var(--navy)] hover:text-white hover:border-[var(--navy)] transition-colors cursor-pointer"
              >
                {industry}
              </Badge>
            ))}
