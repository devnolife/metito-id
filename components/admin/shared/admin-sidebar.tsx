...
export function AdminSidebar({ collapsed = false, onToggleCollapse }: AdminSidebarProps) {
  ...
  const menuItems = [
    ...
    { id: "customers", label: "Pelanggan", icon: Users, href: "/admin/customers" },
    { id: "certifications", label: "Sertifikasi", icon: Award, href: "/admin/certifications" },
    { id: "blog", label: "Blog", icon: FileText, href: "/admin/blog" },
    { id: "contact", label: "Kontak", icon: Phone, href: "/admin/contact" },
    { id: "settings", label: "Pengaturan", icon: Cog, href: "/admin/settings" },
  ]
      ...
      <nav className="p-4 space-y-2">
        {menuItems.map((item) => (
          <Link key={item.id} href={item.href}>
            <Button
              variant="ghost"
              className={`w-full justify-start text-left h-12 transition-all duration-200 ${pathname === item.href
