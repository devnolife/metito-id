...
export async function issueQuotation(quotationId: string, options: IssueOptions = {}) {
  ...
  const issuerName = quotation.createdBy?.name ?? 'Tidak diketahui'
  ...
  for (let attempt = 1; attempt <= MAX_ALLOCATION_ATTEMPTS; attempt += 1) {
    try {
      ...
      // Register surat memuat seluruh surat keluar, termasuk SPH. Pencatatan
      // dijalankan di luar transaksi dan kegagalannya tidak membatalkan
      // penerbitan: nomornya sudah terpakai dan dokumen sudah sah.
      if (issued.numberBase && issued.seq !== null) {
        try {
          await registerQuotationLetter({
            quotationId: issued.id,
            number: issued.numberBase,
            seq: issued.seq,
            letterDate: issued.issuedAt ?? issuedAt,
            subject: issued.subject,
            recipient: issued.customerName,
            issuerName,
            createdById: issued.createdById,
          })
        } catch (error) {
